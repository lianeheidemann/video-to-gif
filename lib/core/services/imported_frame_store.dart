...
import '../painting/frame_painter.dart' show rasterizeCanvas;
...
/// Proporção nativa e janela de conteúdo de uma moldura em SVG.
class SvgFrameMeasurement {
  const SvgFrameMeasurement({
...
NormalizedRect detectContentRect(ByteData raw, int width, int height) {
...
/// Mede um SVG de moldura: rasteriza no tamanho nativo e acha o buraco
/// transparente do meio.
///
/// Serve tanto para a moldura que o usuário importa do aparelho
/// ([SvgFileLoader]) quanto para a que vem solta em `assets/frame`
/// ([SvgAssetLoader]) sem estar registrada na biblioteca — é o que permite
/// soltar um SVG na pasta e ele já aparecer no app.
Future<SvgFrameMeasurement> measureSvgFrame(BytesLoader loader) async {
...
class ImportedFrameStore {
  ...
  Future<ImageFrameAsset> importFrame() async {
    ...
    final medida = await measureSvgFrame(SvgFileLoader(File(path)));
    final contentRect = medida.contentRect;
    final aspectRatio = medida.aspectRatio;
    final nativeWidth = medida.nativeWidth;
